...
description: "Creating high quality NPM packages to share with the community is not hard. It's all about configuring the right tools to helps us keeping code quality high. Let's learn, through a practical exercise, what these tools are and how to configure them."
...
**TL;DR:** In this article, we are going to learn what tools and best practices we should take advantage of when developing NPM packages. We will start from scratch. We will create a GitHub Repository to host our code, then we will pass through multiple interesting and important topics. For example, we will talk about IDEs, we will configure ESLint in our project, and we will even add some nice badges to our README file. [The code that gave life to this article can be found in this GitHub repository](https://github.com/brunokrebs/masks-js).
...
Then, we need to _replace_ the `test` script in our `package.json` file by the following command:
...
> [__Note that__, as we also want to use modern JavaScript in our tests, we used Mocha's `--compilers` option to register Babel as a compiler](http://jamesknelson.com/testing-in-es6-with-mocha-and-babel-6/).
