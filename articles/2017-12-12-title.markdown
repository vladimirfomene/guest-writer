...
The library handles every stage of the process, from the initial HTTP request to the caching of the image just as Glide does.
...
Displaying videos poses to be another difficulty experienced during development. The process and details to take note of can be too numerous to handle. In this category, there are a few viable options available and we will just consider one of them, Exoplayer.
...
In the `onStart` method, we call the `setupPlayer` method:
...
With this, we have a basic implementation of `ExoPlayer`. Google provides a [code-lab](https://codelabs.developers.google.com/codelabs/exoplayer-intro) on how to get started with this library.
...
The first converter is to map to and fro `JSON` while the second is used when we want to deal with primitive data types like type `String`. We then create an interface to show the endpoints that will be accessed :
...
From the snippet, we have an endpoint `/data` that requires a `JsonObject` body for the request. We also have a `ResponseClass` which will be mapped to the expected response body of our request. However, the class is omitted here for brevity. You can use [JsonSchema2Pojo](jsonschema2pojo.org) to map JSON responses to POJOs. We then create a Retrofit client:
