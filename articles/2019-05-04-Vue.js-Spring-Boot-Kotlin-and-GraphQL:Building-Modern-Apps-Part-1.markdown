...
Next, create a helloworld endpoint in your Spring Boot app by creating a `HelloWorldController.kt` in your `com.example.MovieReviewBoard` package. After adding in the line package `com.example.MovieReviewBoard` to the top of that new file, add in the following code:.
