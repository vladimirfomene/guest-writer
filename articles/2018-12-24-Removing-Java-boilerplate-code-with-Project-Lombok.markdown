...
learn how to use this library by developing an example per its features. Last but not the least, you will learn how to incorporate this library in your [Spring Boot](https://spring.io/projects/spring-boot) projects.
...
Now, use Project Lombok's annotations to automagically generate the getters and setters above. After refactoring, the above class will look as follows:
...
Run your application and you should see your two implementations print their hashcodes and the result of comparing objects using their `equals` methods.
...
Notice the check that the above class constructor does on the `clientName` parameter. We can do that check using the @NonNull annotation. To start, create `LombokClient.java` in the `NonNull` package and then fill it with the following code:
...
Notice how with Project Lombok all you have to do is add the @Log annotation to your classes. To test the above implementations, add the `testLog` method to the `src/main/java/TestLombok` class, then comment out all the other methods calls in `TestLombok`'s main class and finally place a call to your `testLog` method in main. Here is the code for `testLog`:
...
In this article you learned how to use Project Lombok's annotations to remove boilerplate code from your project. To learn how to use each annotation, you first created a vanilla Java version of the example and then, refactored it using Project Lombok's annotations. Although this article exposed you to most of Project Lombok's annotations, there is still more to learn like adding options to your annotations and other annotations. To learn more, you can read from [Project Lombok's documentation](https://projectlombok.org/features/all)
